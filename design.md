...
### 7.3.bis Module Code
Trois colonnes élastiques : arborescence `clamp(180px, 18%, 264px)` · éditeur `min-width 280px` · assistant `clamp(300px, 30%, 440px)`, repliable.
**Place insuffisante** (`lib/layout.ts`, mesuré en direct) : l'éditeur garde toujours 280 px ; les panneaux ouverts (arborescence 180, aperçu 280, assistant 300 au minimum) rétrécissent d'abord, puis le **moins récemment ouvert** se replie. L'arborescence repliée devient une barre de 40 px avec ses deux icônes (un clic la rouvre et fait céder un autre panneau) ; un assistant replié faute de place garde son bouton avec une pastille `--accent`. Les largeurs choisies ne sont jamais réécrites : elles reviennent avec la place. À 960 × 640, rail déployé : barre d'icônes · éditeur · assistant. Onglets de fichiers en `text-footnote`, hauteur 28. L'éditeur (CodeMirror 6) n'a **pas** de thème propre : ses couleurs sont mappées sur les tokens (`--color-text`, `--color-accent`, `--font-mono`), donc il suit automatiquement le preset choisi.

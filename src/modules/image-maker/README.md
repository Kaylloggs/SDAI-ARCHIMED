...
|---|---|---|
| OpenRouter | Images API (`/api/v1/images`) : modèles, capacités (`supported_parameters`), prix publiés (`/endpoints`), coût réel (`usage.cost`), crédit de la clé (`/key`) | aucun |
| Google AI Studio | `generateContent` des modèles d'image Gemini : format (`aspectRatio`), taille (`imageSize`) ; pas de coût dans la réponse | application Gemini (gemini.google.com) |
| ChatGPT | (pas d'API branchée) | chatgpt.com |
...
- **Sites officiels** (Gemini sur gemini.google.com pour Nano Banana, ChatGPT sur chatgpt.com, Higgsfield sur higgsfield.ai) : rien n'automatise le site.
  - **Dans ARCHIMED** (« Ouvrir ici ») : le site remplace l'image dans le studio, dans une vue web à part (`browser.rs`, vue enfant de Tauri, fonction `unstable`), avec sa barre (sites, précédent, suivant, recharger, accueil, adresse toujours visible, ouvrir dans le navigateur, fermer). Tauri refuse toute commande venant d'une page distante (aucune capacité `remote`) : le site n'a aucun accès à l'application, et rien n'est injecté dans ses pages. Les fichiers téléchargés vont dans le dossier Téléchargements, comme dans un navigateur ; leur fin est signalée au studio (événement `image-maker:browser`, envoyé à la seule fenêtre principale) et l'image s'importe en un clic depuis le panneau de droite. La vue native, toujours au premier plan, se cache dès qu'une fenêtre modale, un menu ou une liste s'ouvre, quand on quitte le studio ou le module. La session du site reste dans le profil WebView2 de l'application.
  - **Dans votre navigateur** : copier la demande (et l'image si besoin), ouvrir le site, créer, télécharger : l'image apparaît dans la fenêtre (dossier Téléchargements, vérifié toutes les 5 s).
  - Import en nouvelle version de l'image affichée ou en nouvelle image, avec le site d'origine dans son nom. Glisser-déposer et Ctrl+V marchent aussi.
  - Google peut refuser la connexion à un compte dans une application (vue web intégrée) : le site s'ouvre alors dans le navigateur, sans contournement.

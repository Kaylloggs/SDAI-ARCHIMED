...
const appear = (index: number) => ({
  ...
  transition: { duration: duration.base, ease: ease.emphasized, delay: index * 0.035 },
});
    ...
    >
      ...
      >
        <Icon
          size={hero ? 22 : 18}
          strokeWidth={1.75}
          className={cn("text-text-muted transition-colors group-hover:text-text", hero && "text-accent")}
        />
      </span>
      <span className={cn("mt-auto font-semibold", hero ? "pt-8 text-title-1 tracking-[-0.015em]" : "pt-4 text-title-3")}>
        {module.name}
      </span>
      <span className={cn("line-clamp-2 text-text-subtle", hero ? "max-w-[46ch] pt-1 text-body" : "pt-0.5 text-footnote")}>
        {module.description}
      </span>
      {hero && (
        <ArchimedLogo
...
export default function HomeModule() {
  ...
  return (
    <div className="mx-auto flex h-full max-w-[1180px] flex-col px-6 py-6">
      <header className="flex items-end justify-between gap-4 pb-5">
        <div>
          <p className="text-footnote text-text-subtle">SDAI ARCHIMED</p>
          <h1 className="text-display font-semibold tracking-[-0.02em]">{greeting()}</h1>
        </div>
        <p className="hidden text-footnote text-text-subtle md:block">Ctrl K pour tout retrouver</p>
      </header>

      <div className="grid min-h-0 flex-1 auto-rows-[minmax(150px,1fr)] grid-cols-4 gap-3">
        {heroModule && (
          <Tile
            module={heroModule}
            index={0}
        ...
        >
          ...
          {recent.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
              <ArchimedLogo size={32} tone="mono" className="text-text-subtle" />
              <p className="text-footnote text-text-subtle">Vos conversations apparaîtront ici.</p>
            </div>
          ) : (
            <ul className="flex flex-col gap-1">
              {recent.map((session) => {
                const Icon = session.origin === "code" ? Code2 : MessagesSquare;
                return (
                  <li key={session.id}>
                    <button
                      onClick={() => openSession(session.id, session.origin, session.cwd)}
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-body-sm">{session.title}</span>
                        <span className="block text-caption text-text-subtle">{ago(session.updatedAt)}</span>
                      </span>
                    </button>
                  </li>
                );
